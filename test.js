...
function onFormSubmit(e) {
    var sourceSheet = SpreadsheetApp.getActiveSpreadsheet().getActiveSheet(); // Get the sheet where the form responses are stored
    var row = sourceSheet.getLastRow();
    var newValues = sourceSheet.getRange(row, 1, 1, sourceSheet.getLastColumn()).getValues();
    ...
    var requests = 4;
      ...
      for(var j = 1; j <= requests; j++){
        ...
        var sourceRange = sourceSheet.getRange(row, 1, 1, sourceSheet.getLastColumn()); // Adjust this range as per your requirement
  ...
  function appendNewestRow() {
  
    row = 12; //Change based on newest row -----------------------------------------------------
